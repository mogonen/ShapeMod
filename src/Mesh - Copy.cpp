...
/*
VertexPtr * Vertex::sortVerts(VertexPtr v0){
	VertexPtr * vs = new VertexPtr[verts.size()-1];
	Vec3 x = (v0->getP() - getP()).normalize();
	Vec3 y = (x%Vec3(0,0,1)).normalize();
	int sz = 0; 
	for(std::set<VertexPtr>::iterator itv = verts.begin(); itv != verts.end(); itv++)
		if (*itv != v0)
			vs[sz++] = (*itv);
	double * deg = new double[sz];
	for(int i = 0; i<sz; i++ ){
		Vec3 n((vs[i]->getP() - getP()).normalize());
		deg[i] = (n * x);
		if (n*y<0)
			deg[i] = -2 - deg[i];
	}
